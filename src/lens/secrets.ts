...
const ENTROPY_THRESHOLDS = { hex: 3.2, b64: 4.2, alpha: 3.5, any: 3.8 };
...
function isHighEntropy(value: string, minLen = 20): boolean {
  ...
  if (/^[0-9a-fA-F]+$/.test(value)) return e > ENTROPY_THRESHOLDS.hex;
  if (/^[A-Za-z0-9+/=_\-]+$/.test(value)) return e > ENTROPY_THRESHOLDS.b64;
  return e > ENTROPY_THRESHOLDS.any;
}

function isFalsePositive(value: string): boolean {
  const v = value.toLowerCase().trim();
  if (FP_BLACKLIST.has(v)) return true;
  if (new Set(v).size < 4) return true;
  if (/^[a-z_\-]+$/.test(v)) return true;
  if (/^[\d.]+$/.test(v)) return true;
  if (new Set(v.replace(/[-_]/g, "")).size < 3) return true;
  return false;
}
...
export function containSecrets(input: string): boolean {
  for (const { pattern, severity } of SECRET_PATTERNS) {
    ...
    if (severity === "MEDIUM" && !isHighEntropy(value)) continue;
    return true;
  }
