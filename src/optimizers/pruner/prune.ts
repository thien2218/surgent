...
function hasNinetyPercentCoverage(range: [number, number], candidates: ResourceResult[]): boolean {
  ...
  coveredLines += coveredEnd - coveredStart + 1;
  return coveredLines * 10 >= (range[1] - range[0] + 1) * 9;
}
...
function linkSupersededResults(resourceResults: ResourceResult[]): boolean {
  ...
  for (let index = resourceResults.length - 1; index >= 0; index -= 1) {
    ...
    if (hasNinetyPercentCoverage(resourceResult.range, coveringReads)) {
      changed =
        setRepeat(
